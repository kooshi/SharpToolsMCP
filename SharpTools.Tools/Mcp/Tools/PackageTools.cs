...
public static class PackageTools
{
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            bool packageExists = await ValidatePackageAsync(nugetPackageId, version, logger, cancellationToken);
            ...
            LegacyNuGetPackageReader.PackageFormat packageFormat = LegacyNuGetPackageReader.DetectPackageFormat(projectPath);
            string action = "added";
            LegacyNuGetPackageReader.ProjectPackageInfo projectPackages = LegacyNuGetPackageReader.GetPackagesForProject(projectPath);

            // Check if package already exists to determine if we're adding or updating
            LegacyNuGetPackageReader.PackageReference? existingPackage = projectPackages.Packages.FirstOrDefault(p =>
            ...
            if (packageFormat == LegacyNuGetPackageReader.PackageFormat.PackageReference)
            {
                await UpdatePackageReferenceAsync(
                    projectPath, nugetPackageId, version, existingPackage != null, documentOperations, logger, cancellationToken);
            }
            else
            {
                string? packagesConfigPath = projectPackages.PackagesConfigPath
                    ?? throw new McpException("packages.config path not found.");
                await UpdatePackagesConfigAsync(
                    packagesConfigPath, nugetPackageId, version, existingPackage != null, documentOperations, logger, cancellationToken);
            }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            NuGet.Common.ILogger nugetLogger = NullLogger.Instance;
            ...
            NuGetVersion targetVersion = NuGetVersion.Parse(version);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            (string projectContent, _) = await documentOperations.ReadFileAsync(projectPath, false, cancellationToken);
            XDocument xDoc = XDocument.Parse(projectContent);

            // Find ItemGroup that contains PackageReference elements or create a new one
            XElement? itemGroup = xDoc.Root?.Elements("ItemGroup")
            ...
            XElement? existingPackage = itemGroup.Elements("PackageReference")
                .FirstOrDefault(pr => string.Equals(pr.Attribute("Include")?.Value, packageId, StringComparison.OrdinalIgnoreCase));

            if (existingPackage != null)
            {
                // Update existing package
                XAttribute? versionAttr = existingPackage.Attribute("Version");
                if (versionAttr != null)
                {
                ...
                else
                {
                    ...
                    XElement? versionElement = existingPackage.Element("Version");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (fileExists)
            {
                (string content, _) = await documentOperations.ReadFileAsync(packagesConfigPath, false, cancellationToken);
            ...
            XElement? packageElement = xDoc.Root?.Elements("package")
