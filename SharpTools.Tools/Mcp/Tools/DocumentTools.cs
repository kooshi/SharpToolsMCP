...
public static class DocumentTools
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            if (documentOperations.FileExists(filePath) == false)
            {
                throw new McpException($"File not found: {filePath}");
            }

            if (documentOperations.IsPathReadable(filePath) == false)
            {
            ...
            try
            {
                (string contents, int lines) = await documentOperations.ReadFileAsync(filePath, true, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            {
                throw new McpException(
                    $"File already exists at {filePath}. Use '{ToolHelpers.SharpToolPrefix}{nameof(ReadRawFromRoslynDocument)}' to understand its contents. " +
                    $"Then you can use '{ToolHelpers.SharpToolPrefix}{nameof(OverwriteRoslynDocument)}' if you decide to overwrite what exists.");
            }

            // Check if path is writable
            if (documentOperations.IsPathWritable(filePath) == false)
            {
            ...
            try
            {
                ...
                bool success = await documentOperations.WriteFileAsync(filePath, content, false, cancellationToken, finalCommitMessage);
                if (success == false)
                {
                ...
                string? solutionPath = solutionManager.CurrentSolution?.FilePath;
                ...
                List<Document>? documents = solutionManager.CurrentSolution?.Projects
                    .SelectMany(p => p.Documents)
                    .Where(d => d.FilePath != null &&
                        d.FilePath.Equals(filePath, StringComparison.OrdinalIgnoreCase))
                    .ToList();

                string projectStatus = "but was not detected by any project";
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            try
            {
                ...
                {
                    logger.LogInformation(
                        "Document found in solution, updating through workspace API: {FilePath}", filePath);

                    // Create a new document text with the updated content
                    SourceText newText = SourceText.From(content);
                    Document newDocument = existingDocument.WithText(newText);
                    Solution solution = newDocument.Project.Solution;
                ...
                else
                {
                    ...
                    bool success = await documentOperations.WriteFileAsync(filePath, content, true, cancellationToken, finalCommitMessage);
                    if (success == false)
                    {
                    ...
                    else
                    {
                        ...
                        Document? document = solutionManager.CurrentSolution?.Projects
                            .SelectMany(p => p.Documents)
                            .FirstOrDefault(d => d.FilePath != null &&
                                d.FilePath.Equals(filePath, StringComparison.OrdinalIgnoreCase));
        ...
        CancellationToken cancellationToken)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            PathInfo pathInfo = documentOperations.GetPathInfo(filePath);
            if (pathInfo.Exists == false)
            {
                throw new McpException($"File not found: {filePath}");
            }

            if (pathInfo.IsReferencedBySolution == false)
            {
            ...
            foreach (TypeDeclarationSyntax typeNode in typeNodes)
            {
                ...
                {
                    ISymbol? declaredSymbol = semanticModel.GetDeclaredSymbol(typeNode, cancellationToken);
                    ...
                    if (declaredSymbol is INamedTypeSymbol declaredNamedTypeSymbol)
                    {
                        ...
                        INamedTypeSymbol? resolvedNamedTypeSymbol = await solutionManager.FindRoslynNamedTypeSymbolAsync(
                            fullyQualifiedName, cancellationToken);
                        ...
                        else if (resolvedNamedTypeSymbol == null
                            && declaredNamedTypeSymbol.TypeKind != TypeKind.Error
                            && declaredNamedTypeSymbol.ContainingAssembly != null)
                        {
                            // Fallback to declared symbol if it's valid but resolution failed
                            logger.LogDebug(
                                "Using originally declared symbol for {SymbolName} as re-resolution failed but declared symbol appears valid.",
                                fullyQualifiedName);
                        ...
                        {
                            logger.LogWarning(
                                "Skipping symbol {SymbolName} from file {FilePath} as it could not be properly resolved " +
                                "to a valid named type symbol with an assembly context. Resolved: {ResolvedIsNull}, " +
                                "Declared TypeKind: {DeclaredTypeKind}, Declared AssemblyNull: {DeclaredAssemblyIsNull}",
                                fullyQualifiedName,
                                filePath,
