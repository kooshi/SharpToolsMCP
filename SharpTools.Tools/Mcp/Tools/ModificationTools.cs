...
public static class ModificationTools
{
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            ISymbol targetSymbol = await ToolHelpers.GetRoslynSymbolOrThrowAsync(solutionManager, fullyQualifiedTargetName, cancellationToken);
            ...
            SyntaxNode syntaxNode = await targetSyntaxRef.GetSyntaxAsync(cancellationToken);
            Document document = ToolHelpers.GetDocumentFromSyntaxNodeOrThrow(solutionManager.CurrentSolution, syntaxNode);
            ...
            if (IsMemberAllowed(typeSymbol, memberSyntax, memberName, cancellationToken) == false)
            {
            ...
            try
            {
                ...
                Solution newSolution = await modificationService.AddMemberAsync(document.Id, typeSymbol, memberSyntax, lineNumberHint, cancellationToken);
                ...
                Document? updatedDocument = solutionManager.CurrentSolution.GetDocument(document.Id);
                ...
                (bool hasErrors, string errorMessages) = await ContextInjectors.CheckCompilationErrorsAsync(
                ...
                SemanticModel? updatedSemanticModel = await updatedDocument.GetSemanticModelAsync(cancellationToken);
                ...
                string baseMessage = $"Successfully added member to {fullyQualifiedTargetName} in {document.FilePath ?? "unknown file"}.\n\n" +
                    (hasErrors == false ? "<errorCheck>No compilation issues detected.</errorCheck>" :
                    ($"{errorMessages}\n" +
    ...
    private static bool IsMemberAllowed(INamedTypeSymbol typeSymbol, MemberDeclarationSyntax newMember, string memberName, CancellationToken cancellationToken)
    {
        ...
        if (newMember is MethodDeclarationSyntax newMethod)
        {
            ...
            foreach (IMethodSymbol existingMethod in existingMethods)
            {
                ...
                {
                    IParameterSymbol existingParam = existingMethod.Parameters[i];
        ...
        else
        {
            ...
            return typeSymbol.GetMembers(memberName).Any(m => m.IsImplicitlyDeclared == false) == false;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            ISymbol symbol = await ToolHelpers.GetRoslynSymbolOrThrowAsync(solutionManager, fullyQualifiedMemberName, cancellationToken);
            ...
            SyntaxNode oldNode = await syntaxRef.GetSyntaxAsync(cancellationToken);
            ...
            Document document = ToolHelpers.GetDocumentFromSyntaxNodeOrThrow(solutionManager.CurrentSolution, oldNode);
            ...
            if (isDelete)
            {
                SyntaxTrivia commentTrivia = SyntaxFactory.Comment(newMemberCode);
                ...
                try
                {
                    Solution newSolution = await modificationService.ReplaceNodeAsync(document.Id, oldNode, emptyNode, cancellationToken);
                    await modificationService.ApplyChangesAsync(newSolution, cancellationToken, finalCommitMessage);
            ...
            try
            {
                Solution newSolution = await modificationService.ReplaceNodeAsync(document.Id, oldNode, newNode, cancellationToken);
                await modificationService.ApplyChangesAsync(newSolution, cancellationToken, finalCommitMessage);
                ...
                string diffResult = ContextInjectors.CreateCodeDiff(oldNode.ToFullString(), newNode.ToFullString());
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            try
            {
                ...
                Solution newSolution = await modificationService.RenameSymbolAsync(symbol, newName, cancellationToken);

                // Check if the operation actually made changes
                SolutionChanges changeset = newSolution.GetChanges(solutionManager.CurrentSolution!);
                int changedDocumentCount = changeset.GetProjectChanges().Sum(p => p.GetChangedDocuments().Count());
                ...
                foreach (DocumentId docId in affectedDocumentIds)
                {
                    if (solutionManager.CurrentSolution != null)
                    {
                        Document? updatedDoc = solutionManager.CurrentSolution.GetDocument(docId);
                        if (updatedDoc != null)
                        {
                            (bool docHasErrors, string docErrorMessages) = await ContextInjectors.CheckCompilationErrorsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            logger.LogInformation(
                "Executing '{ReplaceAllReferences}' for {SymbolName} with text '{ReplacementCode}', filter: {Filter}",
            ...
            try
            {
                ExpressionSyntax? expressionSyntax = SyntaxFactory.ParseExpression(replacementCode);
            ...
            if (string.IsNullOrEmpty(filenameFilter) == false)
            {
            ...
            try
            {
                ...
                Solution newSolution = await modificationService.ReplaceAllReferencesAsync(
                ...
                Solution originalSolution = solutionManager.CurrentSolution;
                SolutionChanges solutionChanges = newSolution.GetChanges(originalSolution);
                int changedDocumentsCount = solutionChanges.GetProjectChanges()
                    .SelectMany(pc => pc.GetChangedDocuments())
                    .Count();
                ...
                string filterMessage = string.IsNullOrEmpty(filenameFilter) ? "" : $" (with filter '{filenameFilter}')";
        ...
        CancellationToken cancellationToken)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            (bool success, string message) = await modificationService.UndoLastChangeAsync(cancellationToken);
            if (success == false)
            {
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            try
            {
                ...
                if (target.Contains("*") || target.Contains("?") || (File.Exists(target) && documentOperations.IsCodeFile(target) == false))
                {
                    ...
                    string? solutionDirectory = Path.GetDirectoryName(originalSolution.FilePath);
                    ...
                    if (target.Contains("*") == false && target.Contains("?") == false && File.Exists(target))
                    {
                        ...
                        if (pathInfo.IsWithinSolutionDirectory == false)
                        {
                        ...
                        if (documentOperations.IsCodeFile(target) == false)
                        {
                            (bool changed, string diff) = await ProcessNonCodeFile(target, regexPattern, replacementText, documentOperations, nonCodeFilesModified, cancellationToken);
                            if (changed)
                            {
                    ...
                    else
                    {
                        ...
                        foreach (FileInfo file in allFiles)
                        {
                            ...
                            {
                                PathInfo pathInfo = documentOperations.GetPathInfo(file.FullName);

                                // Skip files in unsafe directories or outside solution
                                if (pathInfo.IsWithinSolutionDirectory == false || string.IsNullOrEmpty(pathInfo.WriteRestrictionReason) == false)
                                {
                                ...
                                if (documentOperations.IsCodeFile(file.FullName) == false)
                                {
                                    (bool changed, string diff) = await ProcessNonCodeFile(file.FullName, regexPattern, replacementText, documentOperations, nonCodeFilesModified, cancellationToken);
                                    if (changed)
                                    {
                ...
                Solution newSolution = await modificationService.FindAndReplaceAsync(
                ...
                foreach (DocumentId docId in changedDocuments)
                {
                    Document? document = originalSolution.GetDocument(docId);
                ...
                StringBuilder nonCodeDiffBuilder = new();
                if (nonCodeDiffs.Count > 0)
                {
                    ...
                    foreach (KeyValuePair<string, string> diffEntry in nonCodeDiffs)
                    {
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            ISymbol sourceMemberSymbol = await ToolHelpers.GetRoslynSymbolOrThrowAsync(solutionManager, fullyQualifiedMemberName, cancellationToken);
            ...
            ISymbol destinationSymbol = await ToolHelpers.GetRoslynSymbolOrThrowAsync(solutionManager, fullyQualifiedDestinationTypeOrNamespaceName, cancellationToken);
            ...
            else if (destinationSymbol is INamespaceSymbol nsSym)
            {
                ...
                Project projectForDestination = currentSolution.GetDocument(sourceDocument.Id)!.Project;
                Document? existingDoc = await FindExistingDocumentWithNamespaceAsync(projectForDestination, nsSym, cancellationToken);
                if (existingDoc != null)
                {
                ...
                {
                    Document newDoc = await CreateDocumentForNamespaceAsync(projectForDestination, nsSym, cancellationToken);
            ...
            try
            {
                Document actualDestinationDocument = currentSolution.GetDocument(destinationDocument.Id)
                ...
                else
                {
                    ...
                    {
                        throw new McpException("Destination namespace symbol is null when expected for namespace move.");
                    }
                ...
                Document sourceDocumentInCurrentSolution = currentSolution.GetDocument(sourceDocument.Id)
                    ?? throw new McpException("Source document not found in current solution after adding member to destination.");
                SyntaxNode? syntaxRootOfSourceInCurrentSolution = await sourceDocumentInCurrentSolution.GetSyntaxRootAsync(cancellationToken)
                ...
                SyntaxNode? sourceMemberNodeInCurrentTree = syntaxRootOfSourceInCurrentSolution.FindNode(sourceMemberNode.Span, findInsideTrivia: true, getInnermostNodeForTie: true);
                if (sourceMemberNodeInCurrentTree == null || sourceMemberNodeInCurrentTree is not MemberDeclarationSyntax)
                {
                    ...
                    {
                        logger.LogWarning(
                            "Could not precisely re-locate source member node by original span or by kind/name after destination add. "
                            + "Original span: {Span}. Member kind: {Kind}, Name: {Name}. File: {File}",
                            sourceMemberNode.Span, memberDeclaration.Kind(), memberName, sourceDocumentInCurrentSolution.FilePath);
                        ...
                        else
                        {
                            throw new McpException(
                                $"Critically failed to re-locate source member node '{memberName}' in '{sourceDocumentInCurrentSolution.FilePath}' for removal after modifications. "
                                + $"Original span {sourceMemberNode.Span}. This usually indicates significant tree changes that broke span tracking or the member was unexpectedly altered or removed.");
                        }
                    }
                    else
                    {
                ...
                Document? finalSourceDocumentAfterApply = solutionManager.CurrentSolution?.GetDocument(sourceDocument.Id);
                Document? finalDestinationDocumentAfterApply = solutionManager.CurrentSolution?.GetDocument(destinationDocument.Id);

                StringBuilder errorBuilder = new("<errorCheck>");
                if (finalSourceDocumentAfterApply != null)
                {
                    (bool sourceHasErrors, string sourceErrorMessages) = await ContextInjectors.CheckCompilationErrorsAsync(solutionManager, finalSourceDocumentAfterApply, logger, cancellationToken);
                ...
                if (finalDestinationDocumentAfterApply != null && (finalSourceDocumentAfterApply?.Id.Equals(finalDestinationDocumentAfterApply.Id) ?? true) == false)
                {
                    (bool destHasErrors, string destErrorMessages) = await ContextInjectors.CheckCompilationErrorsAsync(solutionManager, finalDestinationDocumentAfterApply, logger, cancellationToken);
                ...
                string sourceFilePathDisplay = finalSourceDocumentAfterApply?.FilePath ?? sourceDocument.FilePath ?? "unknown source file";
                string destinationFilePathDisplay = finalDestinationDocumentAfterApply?.FilePath ?? destinationDocument.FilePath ?? "unknown destination file";
                string locationInfo = sourceFilePathDisplay == destinationFilePathDisplay
    ...
    private static async Task<Document?> FindExistingDocumentWithNamespaceAsync(Project project, INamespaceSymbol namespaceSymbol, CancellationToken cancellationToken)
    {
        ...
        foreach (Document document in project.Documents)
        {
            ...
            if (root is CompilationUnitSyntax compilationUnit)
            {
                ...
                bool hasNamespace = compilationUnit.Members
    ...
    {
        string namespaceName = namespaceSymbol.ToDisplayString();
        string fileName = string.IsNullOrEmpty(namespaceName) || namespaceSymbol.IsGlobalNamespace
            ? "GlobalNamespace.cs"
        ...
        int counter = 1;
        string projectDirectory = Path.GetDirectoryName(project.FilePath) ?? throw new InvalidOperationException("Project directory not found");
        ...
        string content = namespaceSymbol.IsGlobalNamespace
            ? "// Global namespace file\n"
            : $"namespace {namespaceName} {{\n    // Namespace content\n}}\n";

        Document newDocument = project.AddDocument(fileName, content, filePath: fullPath);
    ...
    private static async Task<Solution> AddMemberToNamespaceAsync(Document document, INamespaceSymbol namespaceSymbol, MemberDeclarationSyntax memberDeclaration, ICodeModificationService modificationService, CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            string namespaceName = namespaceSymbol.ToDisplayString();
            NamespaceDeclarationSyntax? targetNamespace = compilationUnit.Members
                .OfType<NamespaceDeclarationSyntax>()
        ...
        Document formattedDocument = await modificationService.FormatDocumentAsync(changedDocument, cancellationToken);
    ...
    private static async Task<Solution> RemoveMemberFromParentAsync(Document document, SyntaxNode memberNode, ICodeModificationService modificationService, CancellationToken cancellationToken)
    {
        ...
        if (memberNode.Parent is CompilationUnitSyntax compilationUnit)
        {
            ...
            SyntaxList<MemberDeclarationSyntax> newMembers = compilationUnit.Members.Remove(memberDeclaration);
        ...
        else if (memberNode.Parent is NamespaceDeclarationSyntax namespaceDecl)
        {
            ...
            SyntaxList<MemberDeclarationSyntax> newMembers = namespaceDecl.Members.Remove(memberDeclaration);
            NamespaceDeclarationSyntax newNamespace = namespaceDecl.WithMembers(newMembers);
        ...
        else if (memberNode.Parent is TypeDeclarationSyntax typeDecl)
        {
            ...
            SyntaxList<MemberDeclarationSyntax> newMembers = typeDecl.Members.Remove(memberDeclaration);
            TypeDeclarationSyntax newType = typeDecl.WithMembers(newMembers);
        ...
        Document formattedDocument = await modificationService.FormatDocumentAsync(newDocument, cancellationToken);
        return formattedDocument.Project.Solution;
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            (string originalContent, _) = await documentOperations.ReadFileAsync(filePath, false, cancellationToken);
            Regex regex = new(regexPattern, RegexOptions.Multiline);
