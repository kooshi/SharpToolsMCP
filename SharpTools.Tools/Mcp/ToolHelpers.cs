...
internal static class ToolHelpers
{
    ...
    public static void EnsureSolutionLoaded(ISolutionManager solutionManager)
    {
        if (solutionManager.IsSolutionLoaded == false)
        {
            throw new McpException(
                $"No solution is currently loaded. Please use "
                + $"'{SharpToolPrefix}{nameof(Tools.SolutionTools.LoadSolution)}' first.");
        }
    }
    ...
    /// </summary>
    public static void EnsureSolutionLoadedWithDetails(
        ...
        string operationName)
    {
        if (solutionManager.IsSolutionLoaded == false)
        {
            ...
            throw new McpException(
                $"No solution is currently loaded. Please use "
                + $"'{SharpToolPrefix}{nameof(Tools.SolutionTools.LoadSolution)}' before calling '{operationName}'.");
        }
    }

    private const string FqnHelpMessage =
        $" Try `{ToolHelpers.SharpToolPrefix}{nameof(Tools.AnalysisTools.SearchDefinitions)}`,"
        + $" `{ToolHelpers.SharpToolPrefix}{nameof(Tools.AnalysisTools.GetMembers)}`,"
        + $" or `{ToolHelpers.SharpToolPrefix}{nameof(Tools.DocumentTools.ReadTypesFromRoslynDocument)}`"
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ISymbol? symbol = await solutionManager.FindRoslynSymbolAsync(
                fullyQualifiedSymbolName, cancellationToken);
            return symbol ?? throw new McpException(
                $"Roslyn symbol '{fullyQualifiedSymbolName}' not found in the current solution."
                + FqnHelpMessage);
        }
        ...
        catch (Exception ex) when ((ex is McpException) == false)
        {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            INamedTypeSymbol? symbol = await solutionManager.FindRoslynNamedTypeSymbolAsync(
                fullyQualifiedTypeName, cancellationToken);
            return symbol ?? throw new McpException(
                $"Roslyn named type symbol '{fullyQualifiedTypeName}' not found in the current solution."
                + FqnHelpMessage);
        }
        catch (OperationCanceledException)
        {
        ...
        {
            throw new McpException(
                $"Error finding Roslyn named type symbol '{fullyQualifiedTypeName}': {ex.Message}");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            Type? type = await solutionManager.FindReflectionTypeAsync(
                fullyQualifiedTypeName, cancellationToken);
            return type ?? throw new McpException(
                $"Reflection type '{fullyQualifiedTypeName}' not found in loaded assemblies."
                + FqnHelpMessage);
        }
        catch (OperationCanceledException)
        {
    ...
    public static string GetRoslynSymbolModifiersString(ISymbol symbol)
    {
        ...
        if (symbol.IsAbstract && symbol.Kind != SymbolKind.NamedType)
        {
            parts.Add("abstract"); // Type abstract handled by TypeKind
        }
        if (symbol.IsSealed && symbol.Kind != SymbolKind.NamedType)
        {
            parts.Add("sealed");     // Type sealed handled by TypeKind
        }
        if (symbol.IsVirtual)
        {
            parts.Add("virtual");
        ...
        switch (symbol)
        {
            ...
            case IFieldSymbol fieldSymbol:
                ...
                {
                    parts.Add("const"); // Though 'const' implies static
                }
                break;
            case IPropertySymbol propertySymbol:
                if (propertySymbol.IsReadOnly)
                {
                    parts.Add("readonly"); // Getter only, or init-only setter
                }
                break;
            case INamedTypeSymbol typeSymbol: // For types, abstract/sealed are part of their kind
                if (typeSymbol.IsReadOnly)
                {
                    parts.Add("readonly"); // readonly struct/ref struct
                }
    ...
    private static string ReflectionAccessibilityToString(Type type)
    {
        ...
        {
            return "internal"; // Top-level non-public is internal by default
        }
        return "";
    }
    ...
    public static string GetReflectionMemberModifiersString(MemberInfo memberInfo)
    {
        ...
        switch (memberInfo)
        {
            case MethodInfo mi:
                ...
                if (mi.IsVirtual && mi.IsFinal == false && mi.IsAbstract == false)
                {
                    parts.Add("virtual");
                }
                if (mi.IsVirtual && mi.IsFinal)
                {
                    parts.Add("sealed override"); // Or just "sealed" if not overriding
                }
                else
                {
                    ...
                    try
                    {
                        if (mi.GetBaseDefinition() != mi && mi.IsVirtual == false)
                        {
                            parts.Add("override"); // Non-virtual override (interface implementation)
                        }
                        else if (mi.GetBaseDefinition() != mi)
                        {
                            parts.Add("override");
                        }
                    }
                    catch (NotSupportedException)
                    {
                        ...
                        if (mi.IsVirtual && mi.IsAbstract == false)
                        {
                            parts.Add("override");
                        }
                    }
                ...
                try
                {
                    if (mi.IsDefined(typeof(AsyncStateMachineAttribute), false))
                    {
                        parts.Add("async");
                    }
                }
                ...
                if ((mi.MethodImplementationFlags & MethodImplAttributes.InternalCall) != 0 ||
                    (mi.MethodImplementationFlags & MethodImplAttributes.Native) != 0)
                {
                    parts.Add("extern");
                }
                break;
            case ConstructorInfo ci: // Constructors have accessibility and can be static (type initializers)
                ...
                break;
            case FieldInfo fi:
                if (fi.IsStatic && fi.IsLiteral == false)
                {
                    parts.Add("static"); // const fields are implicitly static
                }
                if (fi.IsInitOnly)
                {
                    parts.Add("readonly");
            ...
            case PropertyInfo pi:
                MethodInfo? accessor = pi.GetAccessors(true).FirstOrDefault();
                if (accessor != null)
                {
                    ...
                    if (accessor.IsVirtual && accessor.IsFinal == false && accessor.IsAbstract == false)
                    {
                        parts.Add("virtual");
                    }
                    ...
                    else
                    {
                        ...
                        try
                        {
                            if (accessor.GetBaseDefinition() != accessor && accessor.IsVirtual == false)
                            {
                                parts.Add("override");
                            }
                        ...
                        catch (NotSupportedException)
                        {
                            // For MetadataLoadContext, we can't check GetBaseDefinition
                            // Infer override status from best available information
                            if (accessor.IsVirtual && accessor.IsAbstract == false)
                            {
                                parts.Add("override");
                            }
                        }
                ...
                break;
            case EventInfo ei:
                MethodInfo? addAccessor = ei.GetAddMethod(true);
                if (addAccessor != null)
                {
                    if (addAccessor.IsStatic)
                    {
                        parts.Add("static");
                    }
                    if (addAccessor.IsAbstract)
                    {
                        parts.Add("abstract");
                    }
                    if (addAccessor.IsVirtual && addAccessor.IsFinal == false && addAccessor.IsAbstract == false)
                    {
                        parts.Add("virtual");
                    }
                    if (addAccessor.IsVirtual && addAccessor.IsFinal)
                    {
                        parts.Add("sealed override");
                    }
                    else
                    {
                        ...
                        try
                        {
                            if (addAccessor.GetBaseDefinition() != addAccessor && addAccessor.IsVirtual == false)
                            {
                                parts.Add("override");
                            }
                            else if (addAccessor.GetBaseDefinition() != addAccessor)
                            {
                                parts.Add("override");
                            }
                        }
                        catch (NotSupportedException)
                        {
                            // For MetadataLoadContext, we can't check GetBaseDefinition
                            // Infer override status from best available information
                            if (addAccessor.IsVirtual && addAccessor.IsAbstract == false)
                            {
                                parts.Add("override");
                            }
                        }
        ...
        return string.Join(" ", parts.Where(p => string.IsNullOrEmpty(p) == false).Distinct());
    }
    ...
    public static string GetReflectionTypeModifiersString(Type type)
    {
        ...
        if (type.IsValueType && type.IsDefined(typeof(IsReadOnlyAttribute), false))
        {
            parts.Add("readonly"); // readonly struct
        }
        if (type.IsByRefLike)
        {
            parts.Add("ref"); // ref struct
    ...
    public static string GetReflectionTypeKindString(Type type)
    {
        ...
        if (type.IsValueType && type.IsPrimitive == false && type.IsEnum == false
            && type.FullName!.StartsWith("System.Nullable") == false)
        {
            return "Struct";
        }
        if (type.IsClass)
