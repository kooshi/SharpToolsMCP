...
internal static class ContextInjectors
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            sb.AppendLine(
                $"<compilationErrors note=\"If the fixes for these errors are simple, use "
                + $"`{ToolHelpers.SharpToolPrefix}{nameof(ModificationTools.FindAndReplace)}`\">");
            // First add member access errors (highest priority as this is what we're focusing on)
            foreach (Diagnostic error in memberAccessErrors)
            {
                FileLinePositionSpan lineSpan = error.Location.GetLineSpan();
                sb.AppendLine(
                    $"  {error.Severity}: {error.Id} - {error.GetMessage()} at line "
                    + $"{lineSpan.StartLinePosition.Line + 1}, column {lineSpan.StartLinePosition.Character + 1}");
            }
            // Then add other errors and warnings
            foreach (Diagnostic diag in diagnostics.Except(memberAccessErrors))
            {
                FileLinePositionSpan lineSpan = diag.Location.GetLineSpan();
                sb.AppendLine(
                    $"  {diag.Severity}: {diag.Id} - {diag.GetMessage()} at line "
                    + $"{lineSpan.StartLinePosition.Line + 1}, column {lineSpan.StartLinePosition.Character + 1}");
            }
    ...
    public static string CreateCodeDiff(string oldCode, string newCode)
    {
        ...
        DiffPlex.DiffBuilder.Model.DiffPaneModel diff = InlineDiffBuilder.Diff(strippedOldCode, strippedNewCode);
        ...
        string diffResult = diffBuilder.ToString();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (DocumentId docId in changedDocuments)
        {
            ...
            Document? oldDoc = originalSolution.GetDocument(docId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            IEnumerable<SymbolCallerInfo> callerInfos =
                await codeAnalysisService.FindCallersAsync(methodSymbol, cancellationToken);
            foreach (SymbolCallerInfo callerInfo in callerInfos)
            {
            ...
            IEnumerable<ISymbol> outgoingSymbols =
                await codeAnalysisService.FindOutgoingCallsAsync(methodSymbol, cancellationToken);
        ...
        IEnumerable<string> randomizedCallers = callers.OrderBy(_ => random.Next()).Take(20);
        foreach (string caller in randomizedCallers)
        {
        ...
        foreach (string callee in callees.OrderBy(_ => random.Next()).Take(20))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            IEnumerable<ReferencedSymbol> references =
                await codeAnalysisService.FindReferencesAsync(typeSymbol, cancellationToken);
            foreach (ReferencedSymbol reference in references)
            {
                ...
                foreach (ReferenceLocation location in reference.Locations)
                {
                    ...
                    SemanticModel? semanticModel =
                        await location.Document.GetSemanticModelAsync(cancellationToken);
                    if (semanticModel == null)
                    {
                        continue;
                    }

                    ISymbol? symbol = semanticModel.GetEnclosingSymbol(
                        location.Location.SourceSpan.Start, cancellationToken);
                    while (symbol != null && (symbol is INamedTypeSymbol) == false)
                    {
                        symbol = symbol.ContainingSymbol;
                    }

                    if (symbol is INamedTypeSymbol referencingType &&
                        SymbolEqualityComparer.Default.Equals(referencingType, typeSymbol) == false)
                    {
        ...
        foreach (string referencingType in referencingTypes.OrderBy(_ => random.Next()).Take(20))
        {
        ...
        foreach (string referencedType in referencedTypes.OrderBy(_ => random.Next()).Take(20))
        {
