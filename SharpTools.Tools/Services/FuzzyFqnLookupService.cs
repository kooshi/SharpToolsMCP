...
public class FuzzyFqnLookupService(ILogger<FuzzyFqnLookupService> logger) : IFuzzyFqnLookupService
{
    ...
    /// <inheritdoc />
    public async Task<IEnumerable<FuzzyMatchResult>> FindMatchesAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (Project project in solutionManager.CurrentSolution.Projects)
        {
            ...
            foreach (Document document in project.Documents)
            {
                ...
                SemanticModel? semanticModel = await solutionManager.GetSemanticModelAsync(
                    document.Id,
                    cancellationToken);
                ...
                {
                    _logger.LogWarning(
                        "Could not get semantic model for document {DocumentPath}",
                        document.FilePath);
                    continue;
                }
        ...
        allRelevantSymbols = new HashSet<ISymbol>(
            ...
            SymbolEqualityComparer.Default); // Remove all but one from each group

        _logger.LogDebug(
            "Filtered {PrefilterCount} symbols to {SymbolCount} after removing duplicates",
            prefilter,
            allRelevantSymbols.Count);

        // Match symbols against the fuzzy FQN
        foreach (ISymbol symbol in allRelevantSymbols)
        {
            ...
            (double score, string reason) = CalculateMatchScore(trimmedFuzzyFqn, symbol, canonicalFqn, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (ISymbol member in containerSymbol.GetMembers())
        {
            ...
            switch (member.Kind)
            {
                ...
                case SymbolKind.Property:
                    IPropertySymbol prop = (IPropertySymbol)member;
                ...
                case SymbolKind.Event:
                    IEventSymbol evt = (IEventSymbol)member;
        ...
        CancellationToken cancellationToken)
    {
                ...
                || methodSymbol.MethodKind == MethodKind.StaticConstructor))
        {
            ...
            if (userInputNoParams.Equals(
                typeFullName + "." + methodSymbol.ContainingType.Name,
        ...
        if (userInputWithNormalizedGenerics.Equals(
            canonicalFqnWithNormalizedGenerics,
            StringComparison.OrdinalIgnoreCase))
        {
        ...
        string? userNestedFixed = TryFixNestedTypeSeparator(
            userInputNoGenerics,
            canonicalFqnNoGenerics,
            cancellationToken);

        if (userNestedFixed != null
            && userNestedFixed.Equals(canonicalFqnNoGenerics, StringComparison.OrdinalIgnoreCase))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        int canonicalLastPlusOrDot = Math.Max(
            canonicalFqnPart.LastIndexOf('+'),
            canonicalFqnPart.LastIndexOf('.'));
        ...
        else if ((string.IsNullOrEmpty(userMemberName) && string.IsNullOrEmpty(canonicalMemberName))
            || (userLastDot == -1 && canonicalLastPlusOrDot == -1))
        {
    ...
    public static string GetSearchableString(ISymbol symbol)
    {
        ...
        if (symbol is IMethodSymbol methodSymbol)
        {
            ...
            string methodNameAndType = methodSymbol.ToDisplayString(fqn);

            // Get param types only
            string parameters = string.Join(", ", methodSymbol.Parameters
            ...
            // Combine all parts
            string signature = methodNameAndType + "(" + parameters + ")";
    ...
    private async Task LogAmbiguityDetailsAsync(
        string fuzzyFqnInput,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning(
                "Ambiguity detected for input '{FuzzyFqn}': Found {HighScoreCount} high-scoring matches (>= {Threshold})",
                fuzzyFqnInput,
                highScoreMatches.Count,
                HighScoreThreshold);
            ...
            {
                _logger.LogWarning(
                    "Cross-project ambiguity detected: Matches span {ProjectCount} projects",
                    matchesByProject.Count);

                foreach (IGrouping<string, FuzzyMatchResult> projectGroup in matchesByProject)
                {
                    _logger.LogInformation(
                        "Project '{ProjectName}' has {MatchCount} ambiguous matches",
                        projectGroup.Key,
                        projectGroup.Count());
                }
            }
            ...
            {
                FuzzyMatchResult match = detailedMatches[i];
    ...
    private void LogDetailedMatchInfoAsync(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ISymbol symbol = match.Symbol;
            string projectName = GetProjectName(symbol);
            string symbolKind = GetSymbolKindString(symbol);
            string containingType = GetContainingTypeInfo(symbol);
            string assemblyName = GetAssemblyName(symbol);
            string location = GetLocationInfo(symbol);
            string formattedSignature = CodeAnalysisService.GetFormattedSignatureAsync(symbol, false);

            _logger.LogWarning(
                "Ambiguous Match #{Rank}: Score={Score:F3}, Reason='{Reason}'",
                rank,
                match.Score,
                match.MatchReason);
            ...
            if (string.IsNullOrEmpty(containingType) == false)
            {
            ...
            string modifiers = GetSymbolModifiers(symbol);
            _logger.LogInformation("    Accessibility: {Accessibility}", accessibility);

            if (string.IsNullOrEmpty(modifiers) == false)
            {
    ...
    private void LogTypeSpecificDetails(ISymbol symbol)
    {
        switch (symbol)
        {
            ...
            case IMethodSymbol method:
                ...
                {
                    string parameterTypes = string.Join(", ", method.Parameters.Select(p => p.Type.ToDisplayString()));
    ...
    private void LogAmbiguitySummary(
        string fuzzyFqnInput,
        List<FuzzyMatchResult> allResults,
        List<FuzzyMatchResult> highScoreMatches,
        List<FuzzyMatchResult> perfectMatches)
    {
        ...
        _logger.LogInformation(
            "  Perfect matches (>= {PerfectThreshold:F3}): {PerfectCount}",
            PerfectMatchScore - 0.01,
            perfectMatches.Count);
        _logger.LogInformation("  High-scoring matches (>= 0.8): {HighScoreCount}", highScoreMatches.Count);

        // Score distribution
        (double upper, double lower, string label)[] scoreRanges =
        [
            (1.0, 0.95, "Excellent"),
            (0.95, 0.9, "Very Good"),
            (0.9, 0.8, "Good"),
        ...
        foreach ((double upper, double lower, string label) in scoreRanges)
        {
            int count = allResults.Count(r => r.Score < upper && r.Score >= lower);

            if (count > 0)
            {
                _logger.LogInformation(
                    "  {Label} matches ({Lower:F1}-{Upper:F1}): {Count}",
                    label,
                    lower,
                    upper,
                    count);
        ...
        if (symbolKinds.Count != 0)
        {
            ...
            foreach (IGrouping<string, FuzzyMatchResult> kind in symbolKinds)
            {
        ...
        if (projects.Count > 1)
        {
            ...
            foreach (IGrouping<string, FuzzyMatchResult> project in projects)
            {
