...
public class GitService(ILogger<GitService> logger) : IGitService
{
    ...
    public async Task<bool> IsRepositoryAsync(string solutionPath, CancellationToken cancellationToken = default)
    {
        return await Task.Run(() =>
        {
            try
            {
                ...
                string? repositoryPath = Repository.Discover(solutionDirectory);
                return string.IsNullOrEmpty(repositoryPath) == false;
            }
            catch (Exception ex)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        return await Task.Run(() =>
        {
            try
            {
                string? repositoryPath = GetRepositoryPath(solutionPath);
        ...
        CancellationToken cancellationToken = default)
    {
        await Task.Run(() =>
        {
            try
            {
                ...
                // Create commit
                Signature signature = GetCommitSignature(repository);
                Commit commit = repository.Commit(commitMessage, signature, signature);
    ...
    private Signature GetCommitSignature(Repository repository)
    {
        try
        {
            ...
            string name = config.Get<string>("user.name")?.Value ?? "SharpTools";
            string email = config.Get<string>("user.email")?.Value ?? "sharptools@localhost";
        ...
        CancellationToken cancellationToken = default)
    {
        return await Task.Run(() =>
        {
            try
            {
                ...
                Commit currentCommit = repository.Head.Tip;
                Branch newBranch = repository.CreateBranch(branchName, currentCommit);
        ...
        CancellationToken cancellationToken = default)
    {
        return await Task.Run(() =>
        {
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "Could not find commits for diff: Old {OldSha}, New {NewSha}",
                        oldCommitSha?[..8] ?? "null",
                        newCommitSha?[..8] ?? "null");
                    return string.Empty;
                }
                ...
                // Compare the trees
                TreeChanges comparison = repository.Diff.Compare<TreeChanges>(
                    oldCommit.Tree,
                    newCommit.Tree);

                foreach (TreeEntryChanges change in comparison)
                {
                    diffOutput.AppendLine($"{change.Status}: {change.Path}");

                    // Get detailed patch for each file
                    Patch patch = repository.Diff.Compare<Patch>(
                        oldCommit.Tree,
                        newCommit.Tree,
                        new[] { change.Path },
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error getting diff for solution at {SolutionPath}",
                    solutionPath);
        ...
        CancellationToken cancellationToken = default)
    {
        return await Task.Run(async () =>
        {
            try
            {
                ...
                if (currentBranch.StartsWith(SharpToolsBranchPrefix, StringComparison.OrdinalIgnoreCase) == false)
                {
                    _logger.LogWarning(
                ...
                string undoBranchName = await CreateUndoBranchAsync(solutionPath, cancellationToken);
                ...
                string diff = await GetDiffAsync(
                    solutionPath,
                    parentCommit.Sha,
                    currentCommit.Sha,
                    cancellationToken);
                ...
                string resultMessage = string.IsNullOrEmpty(undoBranchName) == false
        ...
        CancellationToken cancellationToken = default)
    {
        return await Task.Run(() =>
        {
            try
            {
                ...
                {
                    _logger.LogDebug(
                        "Not on a SharpTools branch, returning empty. Current branch: {BranchName}",
                        currentBranch);
                    return string.Empty;
                }
                ...
                _logger.LogDebug(
                    "Branch origin commit found: {CommitSha}",
                    branchCreationCommit.Sha[..8]);
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error finding branch origin commit for solution at {SolutionPath}",
                    solutionPath);
                return string.Empty;
            }
        }, cancellationToken);
