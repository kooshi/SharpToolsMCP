    ...
    ILogger<SourceResolutionService> logger) : ISourceResolutionService
{
    ...
    public async Task<SourceResult?> ResolveSourceAsync(ISymbol symbol, CancellationToken cancellationToken)
    {
        ...
        {
            SyntaxReference syntaxRef = symbol.DeclaringSyntaxReferences[0];
            SyntaxNode? sourceNode = await syntaxRef.GetSyntaxAsync(cancellationToken);

            if (sourceNode != null)
            {
                SyntaxTree tree = syntaxRef.SyntaxTree;
        ...
        // 2. Try Source Link
        SourceResult? sourceLinkResult = await TrySourceLinkAsync(symbol, cancellationToken);
        ...
        SourceResult? embeddedResult = await TryEmbeddedSourceAsync(symbol, cancellationToken);
        ...
        SourceResult? decompiledResult = await TryDecompilationAsync(symbol, cancellationToken);
    ...
    public async Task<SourceResult?> TrySourceLinkAsync(ISymbol symbol, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            PortableExecutableReference? metadataReference = GetMetadataReferenceForAssembly(assembly);
            ...
            string? dllPath = metadataReference.Display;
            ...
            foreach (CustomDebugInformationHandle customDebugInfoHandle in metadataReader.CustomDebugInformation)
            {
                CustomDebugInformation customDebugInfo = metadataReader.GetCustomDebugInformation(customDebugInfoHandle);
                Guid kind = metadataReader.GetGuid(customDebugInfo.Kind);

                // Source Link kind GUID
                if (kind == new Guid("CC110556-A091-4D38-9FEC-25AB9A351A6A"))
                {
                    BlobReader blobReader = metadataReader.GetBlobReader(customDebugInfo.Value);
            ...
            JsonDocument sourceLinkDoc = JsonDocument.Parse(sourceLinkJson);
            JsonElement urlsElement = sourceLinkDoc.RootElement.GetProperty("documents");
            ...
            foreach (JsonProperty property in urlsElement.EnumerateObject())
            {
                ...
                if (IsPathMatch(symbolDocumentPath, pattern) && string.IsNullOrEmpty(url) == false)
                {
            ...
            string sourceCode = await _httpClient.GetStringAsync(sourceUrl, cancellationToken);
    ...
    public async Task<SourceResult?> TryEmbeddedSourceAsync(ISymbol symbol, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            string? assemblyPath = metadataReference.Display;
            ...
            EmbeddedSourceReader.SourceResult? embeddedSourceInfo = EmbeddedSourceReader.GetEmbeddedSourceForSymbol(symbol);
            ...
            if (string.IsNullOrEmpty(symbolFileName) == false
                && embeddedSources.TryGetValue(symbolFileName, out EmbeddedSourceReader.SourceResult? exactMatch))
            {
            ...
            foreach (KeyValuePair<string, EmbeddedSourceReader.SourceResult> source in embeddedSources)
            {
            ...
            {
                KeyValuePair<string, EmbeddedSourceReader.SourceResult> singleSource = embeddedSources.First();
    ...
    public async Task<SourceResult?> TryDecompilationAsync(ISymbol symbol, CancellationToken cancellationToken)
    {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            ITypeDefinition? typeDef = decompiler.TypeSystem
                            ...
                            IMember? memberDef = typeDef.Members.FirstOrDefault(m => m.Name == memberName);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        try
                        {
                            ...
                            string? containingTypeFullName = symbol.ContainingType?.ToDisplayString(
                                SymbolDisplayFormat.FullyQualifiedFormat);

                            if (string.IsNullOrEmpty(containingTypeFullName) == false)
                            {
                                containingTypeFullName = containingTypeFullName.Replace("global::", "")
                                    .Replace("<", "{")
                                    .Replace(">", "}");

                                string decompiled = decompiler.DecompileTypeAsString(
    ...
    private PortableExecutableReference? GetMetadataReferenceForAssembly(IAssemblySymbol assembly)
    {
        ...
        {
            foreach (PortableExecutableReference reference in project.MetadataReferences.OfType<PortableExecutableReference>())
            {
